...
This version preserves the mature ``_decompress10_6d.py`` machinery.
It uses curve coordinates for deformation, z-dependent tangents, local
multistart Newton correction, and Herglotz acceptance on physical m.
...
from ._deform_coeffs2 import _deform_coeffs_core
...
@njit(cache=numba_cache)
def _residual_jacobian_numba(z_fixed, tau, coeffs, c0, zeta, y):
    """
    Residual and Jacobian entries for the deformation 2x2 system.

    The internal continuation variables are the natural spectral-curve
    coordinates

        (zeta, y),  y = m0(zeta),

    exactly as in the earlier deformation solver. The companion value is

        eta = c0 * y + (c0 - 1) / zeta.

    We use the desingularized deformation constraint

        F2 = (tau*zeta - z) * (c0*y*zeta + c0 - 1)
             + (tau - 1) * zeta.

    This is equivalent to the companion-coordinate relation, but avoids
    continuing directly in eta and avoids explicit division by eta in F2.
    """
...
@njit(cache=numba_cache)
def _curve_tangent_tau_numba(z_fixed, tau, coeffs, c0, zeta, y):
    """
    Tangent d(zeta, y)/d tau along the deformation system at fixed z.
    """
    ...
    # F2_tau = zeta * A + zeta, so J*q_tau = -F_tau.
    rhs1 = 0.0 + 0.0j
    rhs2 = -(zeta * A + zeta)
...
@njit(cache=numba_cache)
def _newton_project_numba(z_fixed, tau, coeffs, c0, zeta0, y0,
    ...
    for it in range(max_iter):
        ...
        while lam >= min_lam:
            ...
            F1_try, F2_try, _a, _b, _c, _d, _det_try = \
                _residual_jacobian_numba(z_fixed, tau, coeffs, c0,
                                         zeta_try, y_try)
...
@njit(cache=numba_cache)
def _predict_heun_numba(z_fixed, tau0, tau1, coeffs, c0, zeta0, y0):
    """
    ...
    dz1, dy1, J111, J112, J121, J122, ok1 = _curve_tangent_tau_numba(
        z_fixed, tau1, coeffs, c0, zeta_e, y_e)
    if not ok1:
        return (0.0 + 0.0j, 0.0 + 0.0j, 1.0e300,
                0.0 + 0.0j, 0.0 + 0.0j, False)
...
# ==========================
# physical local check numba
# ==========================

@njit(cache=numba_cache)
def _physical_m_local_numba(z_fixed, tau, c0, zeta, y, w_min):
    """
    Recover the target non-companion Stieltjes value from curve coordinates.
    This local copy is placed before the continuation kernel so Herglotz
    checks can be used inside the adaptive stepper.
    """

    zeta_safe = zeta
    if abs(zeta_safe) < w_min:
        if zeta_safe == 0:
            zeta_safe = complex(w_min, 0.0)
        else:
            zeta_safe = zeta_safe + (w_min * zeta_safe / abs(zeta_safe))

    z_safe = z_fixed
    if abs(z_safe) < w_min:
        if z_safe == 0:
            z_safe = complex(w_min, 0.0)
        else:
            z_safe = z_safe + (w_min * z_safe / abs(z_safe))

    eta = c0 * y + (c0 - 1.0) / zeta_safe
    c = tau * c0
    return (eta - (c - 1.0) / z_safe) / c
...
@njit(cache=numba_cache)
def _herglotz_ok_numba(z_fixed, tau, c0, zeta, y, w_min):
    """
    Check the recovered physical branch. With the convention
    m(z)=int rho(x)/(x-z) dx, Im m(z)>0 for z in C+.
    """

    m = _physical_m_local_numba(z_fixed, tau, c0, zeta, y, w_min)
...
# =======================
# newton multistart numba
# =======================

@njit(cache=numba_cache)
def _newton_project_multistart_numba(
        z_fixed, tau, coeffs, c0,
        zeta_pred, y_pred, zeta_prev, y_prev,
        max_iter, tol_res, tol_step,
        armijo, min_lam, step_clip,
        log_mode, rel_eps, w_min):
    """
    Local deformation-specific multistart corrector.

    This keeps the v10 predictor-corrector machinery, but tries a few local
    seeds exactly as a per-x operation, so embarrassingly parallel execution is
    preserved. Candidate acceptance is based on the recovered physical
    Stieltjes branch satisfying the Herglotz sign.
    """
    ...
    best_res = 1.0e300
    best_corr = 1.0e300
    best_det = complex(numpy.nan, numpy.nan)
    best_score = 1.0e300
    ...
    for iseed in range(4):
        if iseed == 0:
            ...
            zs = 0.5 * (zeta_pred + zeta_prev)
            ys = 0.5 * (y_pred + y_prev)
        else:
            zs = zeta_prev + 0.5 * (zeta_pred - zeta_prev)
            ys = y_prev + 0.5 * (y_pred - y_prev)

        if (not numpy.isfinite(zs)) or (not numpy.isfinite(ys)):
            continue

        zc, yc, ok_newton, it, res_norm, corr_norm, detJc = \
        ...
        if not ok_newton:
            continue
        if (not numpy.isfinite(zc)) or (not numpy.isfinite(yc)):
            continue
        if abs(yc) < w_min:
            continue
        if not numpy.isfinite(abs(detJc)):
            continue

        h_ok, _m_phys = _herglotz_ok_numba(
            z_fixed, tau, c0, zc, yc, w_min)
        if not h_ok:
            continue

        move_prev = _metric_move_numba(
            zc, yc, zeta_prev, y_prev, log_mode, rel_eps)
        corr_seed = _metric_corr_numba(
            zc, yc, zs, ys, corr_norm, log_mode, rel_eps)

        # Branch-consistency score: prefer the solution closest to the
        # previous curve point; correction size and iteration count only
        # break ties. This mirrors deform5's local continuation prior without
        # any neighbor-in-x repair.
        score = move_prev + 1.0e-3 * corr_seed + 1.0e-12 * float(it)

        if (not best_ok) or (score < best_score):
            best_zeta = zc
            best_y = yc
            best_ok = True
            best_it = it
    ...
    return best_zeta, best_y, best_ok, best_it, best_res, best_corr, best_det
...
@njit(cache=numba_cache)
def _advance_one_point_numba(z_fixed, t0, t1, coeffs, c0, zeta0, y0,
                             max_iter, tol_res, tol_step,
                             armijo, min_lam, step_clip,
                             dt_max, dt_min,
                             pred_atol, pred_rtol,
    ...
    while tau_cur < tau_end:
        ...
        zeta_pred, y_pred, raw_err_pred, detJ0, detJ1, ok_pred = \
            _predict_heun_numba(z_fixed, tau_cur, tau_try, coeffs, c0, zeta, y)
        if not ok_pred:
            rejects += 1
            h *= step_shrink
        ...
        if log_mode:
            ...
            dz0, dy0, _a, _b, _c, _d, ok0 = _curve_tangent_tau_numba(
                z_fixed, tau_cur, coeffs, c0, zeta, y)
            if not ok0:
                rejects += 1
                h *= step_shrink
                if rejects > max_reject:
        ...
        zc, yc, ok_newton, _it, _res_norm, corr_norm, detJc = \
            _newton_project_multistart_numba(
                z_fixed, tau_try, coeffs, c0,
                zeta_pred, y_pred, zeta, y,
                max_iter, tol_res, tol_step,
                armijo, min_lam, step_clip,
                log_mode, rel_eps, w_min)

        move = _metric_move_numba(zc, yc, zeta, y, log_mode, rel_eps)
        corr_measure = corr_norm
        corr_ok = (corr_measure <= corr_factor * max(move, rel_eps))
        det_ok = numpy.isfinite(abs(detJc)) and (abs(detJc) >= det_guard)
        h_ok, _m_phys = _herglotz_ok_numba(
            z_fixed, tau_try, c0, zc, yc, w_min)

        if ok_newton and corr_ok and det_ok and h_ok and (yc != 0.0):
            zeta = zc
            y = yc
            tau_cur = tau_try
            rejects = 0
...
# ==============================
# deform coeffs raw helper numba
# ==============================

@njit(cache=numba_cache)
def _deform_coeffs_raw_numba(a, t, c0):

    a_copy = a.copy()
    a_copy[-1, 0] = 0.0 + 0.0j
    return _deform_coeffs_core(a_copy, t, c0)
...
@njit(cache=numba_cache)
def _zeta_y_from_w_numba(z_fixed, tau, c0, w, w_min):
    """
    Reconstruct natural spectral-curve coordinates from a physical target
    Stieltjes value w.

    This helper is used only as a fallback/reprojection seed. The internal
    continuation state is (zeta, y=m0(zeta)), not the companion variable.
    """

    c = tau * c0
    eta = c * w + (c - 1.0) / z_fixed
    if abs(eta) < w_min:
        if eta == 0:
            eta = complex(w_min, 0.0)
        else:
            eta = eta + (w_min * eta / abs(eta))

    zeta = (z_fixed * eta + 1.0 - tau) / (tau * eta)
    y = (eta - (c0 - 1.0) / zeta) / c0
    return zeta, y


# =========================
# physical from curve numba
# =========================

@njit(cache=numba_cache)
def _physical_from_curve_numba(z_fixed, tau, c0, zeta, y, w_min):
...
# =========================
# curve from eta root numba
# =========================

@njit(cache=numba_cache)
def _curve_from_eta_numba(z_fixed, tau, c0, eta, w_min):

    eta_safe = eta
    if abs(eta_safe) < w_min:
        if eta_safe == 0:
            eta_safe = complex(w_min, 0.0)
        else:
            eta_safe = eta_safe + (w_min * eta_safe / abs(eta_safe))

    zeta = (z_fixed * eta_safe + 1.0 - tau) / (tau * eta_safe)
    y = (eta_safe - (c0 - 1.0) / zeta) / c0
    return zeta, y


# =======================
# physical from eta numba
# =======================

@njit(cache=numba_cache)
def _physical_from_eta_numba(z_fixed, tau, c0, eta):

    c = tau * c0
    return (eta - (c - 1.0) / z_fixed) / c
...
@njit(cache=numba_cache)
def _advance_trajectory_numba(z_fixed, t, coeffs, c0, w0,
                              max_iter, tol_res, tol_step,
                              armijo, min_lam, step_clip,
                              dt_max, dt_min,
                              pred_atol, pred_rtol,
    ...
    for k in range(1, n_t):
        ...
        if (not numpy.isfinite(zeta0)) or (not numpy.isfinite(y0)) or \
                (abs(y0) < w_min):
            zeta0, y0 = _zeta_y_from_w_numba(
                z_fixed, tau[k - 1], c0, W[k - 1], w_min)

        zeta1, y1, success = _advance_one_point_numba(
            z_fixed, t[k - 1], t[k], coeffs, c0, zeta0, y0,
            max_iter, tol_res, tol_step,
            armijo, min_lam, step_clip,
            dt_max, dt_min,
            pred_atol, pred_rtol,
        ...
        if success and numpy.isfinite(zeta1) and numpy.isfinite(y1):
            w = _physical_from_curve_numba(z_fixed, tau[k], c0, zeta1, y1,
                                           w_min)
            h_ok, _m_check = _herglotz_ok_numba(
                z_fixed, tau[k], c0, zeta1, y1, w_min)
            if abs(y1) >= w_min and h_ok:
                w_use = w
                zeta_use = zeta1
                y_use = y1
                ok_use = True
                ...
                if pair_enable and (w.imag <= 0.0):
                    coeffs_t = _deform_coeffs_raw_numba(
                        coeffs, float(t[k]), c0)
                    roots = _roots_fixed_z_numba(coeffs_t, z_fixed)
                    if roots.size >= 2:
                        eta1 = c0 * y1 + (c0 - 1.0) / zeta1
                        idx1 = _nearest_root_index_numba(roots, eta1, -1)
                        ...
                        if idx1 >= 0 and idx2 >= 0:
                            ...
                            if numpy.isfinite(abs(Fmm)) and abs(Fmm) > 0.0:
                                ...
                                if numpy.isfinite(gap_loc) and (gap_loc > 0.0):
                                    ...
                                    m2 = _physical_from_eta_numba(
                                        z_fixed, tau[k], c0, r2)
                                    if (abs(r2 - r1) <= pair_gap_factor *
                                            gap_loc) and (m2.imag > 0.0):

                                        zeta_guess, y_guess = \
                                            _curve_from_eta_numba(
                                                z_fixed, tau[k], c0, r2, w_min)

                                        zeta_proj, y_proj, ok_proj, _itp, \
                                            _resp, _corrp, _detp =  \
                                            _newton_project_numba(
                                                z_fixed, tau[k], coeffs, c0,
                                                zeta_guess, y_guess,
                                                max_iter, tol_res, tol_step,
                                                armijo, min_lam, step_clip)
                                        if ok_proj and \
                                                ...
                                                (abs(y_proj) >= w_min):
                                            w_proj = \
                                                _physical_from_curve_numba(
                                                    z_fixed, tau[k], c0,
                                                    zeta_proj, y_proj, w_min)
...
def _run_chunk(args):
    ...
    for j in range(m):
        Wj, okj = _advance_trajectory_numba(
            z_chunk[j], t, coeffs, float(common['c0']), w0_chunk[j],
...
def deform_newton(z_query, t, coeffs, c0, w0_list=None, max_iter=50,
                  tol=1e-12, armijo=1e-4, min_lam=1e-6, w_min=1e-14,
                  **kwargs):
    """
    Free deformation by geometric continuation on the spectral curve.
    ...
    c0 = float(c0)
    if c0 <= 0.0 or not numpy.isfinite(c0):
        raise ValueError('c0 must be a positive finite scalar.')

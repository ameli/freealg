...
def plot_flow(sizes, x, rho, eig_init, eig_final, xlim=None, ylim=None,
              sharey=False, cmap=None, c_range=None, hist_color=None,
              nbins=(80, 120), label_mode='int', log=False, save=False,
              latex=False):
    """
    ...
    sharey : bool, default=False
        If `True`, the y axis for all axes is shared.
    ...
    with texplot.theme(use_latex=latex):
        ...
        fig, ax = plt.subplots(ncols=3, figsize=(10, 3.1), sharey=sharey)

        ax[0].plot(x, rho[0], color=colors[0], label='Fitted', zorder=1)

        supp_init = [numpy.min(eig_init), numpy.max(eig_init)]
        edges, vals = hist(eig_init, nbins[0], atoms=[0], support=supp_init)
        ...
        edges, vals = hist(eig_final, nbins[1], atoms=[0], support=supp_final)
        ...
        ax[1].set_title(r'(b) Free Deformation')
        ax[2].set_title(
...
def plot_mass(sizes, x, rho, atoms=None, log_x=False, gap=0.7, save=False,
              latex=False):
    """
    ...
    atoms : list of tuples, defalt=None
...
def ridgeplot(sizes, x, rho, eigs=None, xlim=None, text_side='left',
              atom_tol=0.0, cmap=None, c_range=None, hspace=-0.3, scaley=True,
    ...
    with texplot.theme(use_latex=latex):
        ...
        for i in range(len(ax)):
            ...
            if eigs is not None:
                ...
                bins = auto_bins(array, factor=bin_factor)
